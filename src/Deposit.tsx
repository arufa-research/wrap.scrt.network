...
export default function Deposit({
	...
	/* const [sourceCosmJs, setSourceCosmJs] = useState<
		SigningCosmWasmClient | SigningStargateClient | null
	>(null); */
	const [sourceCosmJs, setSourceCosmJs] = useState<any>(null);
	...
	const fetchSourceBalance = async (sourceAddress: string) => {
		...
		try {
			...
			if (token.deposits[selectedChainIndex].from_denom.startsWith("juno1")) {
				console.log("sourceCosmJs: ", sourceCosmJsRef.current);
				...
				/* const result = await (
					sourceCosmJs as SigningCosmWasmClient
				).queryContractSmart(token.address, {
					query: {
						balance: { address: sourceAddress },
					},
				}); */
				const result = await (
					sourceCosmJsRef.current as SigningCosmWasmClient
				).queryContractSmart(token.address, {
					query: {
						balance: { address: sourceAddress },
					},
	...
	useEffect(() => {
		(async () => {
			...
			if (token.deposits[selectedChainIndex].from_denom.startsWith("juno1")) {
				...
				console.log("cosmjs (SigningStargateClient): ", cosmjs);
				sourceCosmJsRef.current = cosmjs;
				setSourceCosmJs(cosmjs);
			}
		})();
						...
						try {
							...
							if (
								...
								const txResponse = await (
									sourceCosmJs as SigningStargateClient
								).sendIbcTokens(
									sourceAddress,
									secretAddress,
									{
